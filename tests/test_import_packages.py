...
def test_import(module_name: str, attr: str | None = None) -> bool:
...
def main():
    ...
    for module_name, attr in core_packages:
        if test_import(module_name, attr):
            print(f"  ✓ {module_name}")
            test_passed += 1
        else:
            print(f"  ✗ {module_name}")
    ...
    if materials_available > 0:
        print(
            f"  Materials packages: {materials_available}/{len(materials_packages)} available"
        )
    print()
    ...
    print(
        f"Optional Materials packages / 可选材料科学包: "
        f"{materials_available}/{len(materials_packages)}"
    )
    print()
